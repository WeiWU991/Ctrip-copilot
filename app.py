...
def get_ai_reply(query, history, kb_text, model_choice):
    ...
    # [PROMPT]: 将“一日游”重新定义为“包车一日游”
    system_prompt = f"""
    Role: Senior Travel Planner & Consultant at Ctrip (High-End Customization Team).
    ...
    [LOGIC BRANCHING - SPECIALIZED FOR CUSTOM TEAM]:
    1. **IF User asks about MULTI-DAY PACKAGES (>4 days)**:
       - **Focus**: Search ONLY in Multi-day package docs.
       - **Action**: Extract Product ID -> URL `https://vacations.ctrip.com/travel/detail/{{ID}}`. 
    
    2. **IF User asks about DAY TOURS / SHORT TRIPS (1-3 days)**:
       - **DEFINITION**: We treat "Day Tours" as **"Private Charter Day Tours" (包车一日游)**.
       - **Strategy**: 
         - **YES**, we have Day Tours! (But explain they are Private Charter, not Join-in Bus).
         - **Pricing**: Apply Charter Formula immediately (Base + 1000 Guide).
         - **Negative Constraint**: We DO NOT sell cheap "Seat-in-coach" (拼团/大巴) tours. If user asks for cheap bus tours, politely upgrade them to Charter.
       - **Action**: Provide a structured "Charter Day Tour" plan (Timeline + Price).

    3. **IF User asks about CUSTOMIZATION / CHARTER**:
       - **Formula**: Base Price (from user input or default ~2500) + **1000 RMB Guide Fee**.
       - **Requirement**: Provide a structured Timeline (09:00 -> 10:00...).
    ...
    <<<REPLY_B>>>
    (Professional Plan:
     - **Recommended Product**: "Private Charter Day Tour" (包车一日游)
     - **Itinerary**: 09:00... 10:00...
     - **Cost Breakdown**: Car Base + Guide Fee (1000).
     - **Note**: "专车专导，行程自由"
     - **LINK** (For Multi-day only): 🔗 [点击查看实时库存 & 价格](https://vacations.ctrip.com/travel/detail/{{ID}})
    )
    <<<END_B>>>
...
def generate_itinerary_text(start, end, optimized_stops, dist, dur, price, model_choice):
    ...
    [INTELLIGENT SCENIC JUDGMENT]:
    1. **Far View (📸 推荐远观)**: Massive natural landmarks (e.g., Mt. Fuji). -> Time: ~30-40 mins.
    2. **Visit (🚶 需入内/深度)**: Temples, Parks, Theme Parks. -> Time: ~1.5 - 3 hours.
